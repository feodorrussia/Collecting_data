...
def save_function(frequency, version, limits):
    ylim = limits[frequency]
    curr_fv_data = dataframe.loc[(dataframe['Frequency'] == frequency) & (dataframe['X_end'] - dataframe['X_start'] - (version - 5) * 4 * 1e-3 < 1.e-9)]
    start_signal = curr_fv_data.iloc[0]
    for i in curr_fv_data.index:
        curr_data = curr_fv_data.iloc[i]
        ...
        plt.plot(data)
        plt.figtext(.125, .9, f"fr = {curr_data['Frequency']}Hz v_{version} Num: {(start_signal['X_start'] - curr_data['X_start'])*1e3//4}\n" +
        ...
        plt.savefig(path_tot + f"/img_{start_signal.name + i}.png")
        plt.clf()
...
if __name__ == '__main__':
    ...
    save_function(18, 0, all_limits)
        # input_args = [(f, v, all_limits) for f in range(16, 66) for v in range(11)]
